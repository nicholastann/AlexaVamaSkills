...
// Replace with your url for other devices: change id for switch or lock on server
const char* resource = "/api/view.php?id=5";
  ...
  */
  if (atoi(clientData->Status)==0){                     //if the server status of the appliance is off or locked 
    if (atoi(clientData->Status) != lastStatus){        //if the server status does not match the status of the appliance currently
      while (digitalRead(8) == LOW){                    // loop until the system reaches a new state
        digitalWrite(9, LOW);                          // phase set to high to turn motor clockwise
    ...
    if (atoi(clientData->Status) != lastStatus){        //if the server status does not match the status of the appliance currently
    while (digitalRead(7) == LOW){                      //loop until system reaches a new state
      digitalWrite(9,HIGH);                              //phase set to low to turn motor counterclockwise
      digitalWrite(5,HIGH);                             //enable set to high to enable motor
...
bool sendChange(const char* host, const char* poster, struct clientData* clientData, int state) {
  DynamicJsonDocument doc(1000);              //sends a status update to the server after manual change
  doc["id"] = 5;
  doc["name"] = "Front Lock";
  doc["type"] = "lock";
  if(state==0){
